...
public class GmailConnector implements MailboxConnector {
    ...
    // Label ID generated by Gmail - retrieved Gmail class instance: gmail.users().labels().list(user).execute().getLabels();
    private static final String MAILBOX_LABEL_PROCESSED = "Label_9223360954603989177";
    ...
    private void ExtractFromMessages(Gmail gmailService, List<Message> inboxMessagesIds) {
        ...
        for (Message messageId : inboxMessagesIds) {
            ...
            modifyLabelsInMailbox(gmailService, messageId);
        }
    }
    ...
    //TODO label id is not its name?
    private void modifyLabelsInMailbox(Gmail gmailService, Message messageId) {
        ModifyMessageRequest modifyMessageRequest = new ModifyMessageRequest();
        modifyMessageRequest.setAddLabelIds(List.of(MAILBOX_LABEL_PROCESSED));
        modifyMessageRequest.setRemoveLabelIds(List.of("INBOX"));
        try {
            gmailService.users().messages().modify("me", messageId.getId(), modifyMessageRequest).execute();
        } catch (IOException e) {
            // TODO logging
        }
    }
